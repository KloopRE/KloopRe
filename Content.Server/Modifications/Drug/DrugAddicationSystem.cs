...
public sealed class DrugAddicationSystem : EntitySystem
{
    ...
    public void UpdateDrugAddication(EntityUid uid, InstantDrugAddicationComponent? component = null)
    {
        ...
        component.TimeLastAppointment += seconds;
    ...
    public void TakeDrug(EntityUid uid, int drugStrenght, float addictionStrenght, float toleranceStrenght, InstantDrugAddicationComponent? component = null)
    {
        ...
        {
            AddAddictionLevel(uid, addictionStrenght * drugStrenght / MaxDrugStr, component);
            AddTolerance(uid, toleranceStrenght * drugStrenght / MaxDrugStr, component);
            float randomFactor = Random.Shared.Next(10, 30) * drugStrenght;
            component.TimeLastAppointment -= randomFactor;
    ...
    public void RunEffects(EntityUid uid, InstantDrugAddicationComponent? component = null)
    {
        ...
        if (component.WithdrawalLevel >= 25 && component.WithdrawalLevel < 50)
        {
            if (component.IsTimeSendMessage)
            {
        ...
        if (component.WithdrawalLevel >= 50 && component.WithdrawalLevel < 75)
        {
            if (component.IsTimeSendMessage)
            {
        ...
        if (component.WithdrawalLevel >= 75)
        {
            if (component.IsTimeSendMessage)
            {
    ...
    private void LowEffects(EntityUid uid, InstantDrugAddicationComponent component)
    {
        if (TryComp<StaminaComponent>(uid, out var stamina))
            stamina.StaminaDamage += component.WithdrawalLevel;
    }
    ...
    public float CalculateThresholdTime(InstantDrugAddicationComponent component)
    {
        ...
        component.SomeThresholdTime = Math.Clamp(thresholdTime, 30, 600);

        return component.SomeThresholdTime; // от 30 секунд до 10 минут
    }
}
